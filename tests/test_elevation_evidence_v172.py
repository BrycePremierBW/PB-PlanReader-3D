...
Covers:
  - ElevationOpening dataclass
  - Opening detection from elevation rectangles (sizing, label extraction)
  - Correlation scoring and matching
  - Enrichment via merge_opening_evidence
...
class TestElevationOpeningDataclass(unittest.TestCase):
    ...
    def test_defaults(self):
        ...
        self.assertEqual(e.sill_m, 0.0)
        self.assertEqual(e.head_m, 0.0)
...
class TestCorrelationScore(unittest.TestCase):
    ...
    def test_no_side_on_instance(self):
        inst = _inst(side="")
        elev = _elev(side="North")
        sc = _correlation_score(inst, elev)
        # Should still get some score from width match
    ...
    def test_label_bonus(self):
        ...
        inst2 = _inst(mark="W01", side="North", width=0.82)
        elev2 = _elev(side="North", w=0.82, label="W01")
        sc1 = _correlation_score(inst1, elev1)
        sc2 = _correlation_score(inst2, elev2)
        # Same structural match, but label agreement gives bonus
        self.assertGreaterEqual(sc1, sc2)

    def test_wall_ref_match(self):
        inst = _inst(wall_ref="N01", side="North", width=0.82)
        elev = _elev(side="North", w=0.82)
        sc = _correlation_score(inst, elev)
        self.assertGreater(sc, 0.5)

    def test_wall_ref_mismatch(self):
        inst = _inst(wall_ref="N01", side="North", width=0.82)
        elev = _elev(side="East", w=0.82)
        sc = _correlation_score(inst, elev)
        self.assertEqual(sc, 0.0)
...
class TestDetectElevationOpenings(unittest.TestCase):
    ...
    def test_basic_detection(self):
        ...
        words = []
        result = detect_elevation_openings(
            elevation_page_no=5,
            elevation_side="North",
            rects=rects,
            words=words,
    ...
    def test_filters_too_small(self):
        rects = [_rect(100, 100, 110, 110)]  # 10px × 10px = 0.1m × 0.1m
        result = detect_elevation_openings(1, "North", rects, [], 100.0)
        self.assertEqual(len(result), 0)

    def test_filters_too_large(self):
        rects = [_rect(0, 0, 800, 600)]  # 8m × 6m
        result = detect_elevation_openings(1, "North", rects, [], 100.0)
        self.assertEqual(len(result), 0)

    def test_multiple_rects(self):
        rects = [
            _rect(100, 100, 300, 500),  # 2m × 4m
            _rect(400, 100, 600, 300),  # 2m × 2m
        ]
        result = detect_elevation_openings(1, "North", rects, [], 100.0)
    ...
    def test_negative_scale(self):
        rects = [_rect(100, 100, 300, 500)]
        result = detect_elevation_openings(1, "North", rects, [], -1.0)
        self.assertEqual(len(result), 0)
    ...
    def test_missing_bbox(self):
        rects = [{"confidence": 0.5}]  # no bbox key
        result = detect_elevation_openings(1, "North", rects, [], 100.0)
        self.assertEqual(len(result), 0)


class TestCorrelateElevationToPlan(unittest.TestCase):
    """correlate_elevation_to_plan() — matching + enrichment."""

    def test_basic_match(self):
        """Elevation matches plan instance by side + width."""
        inst = _inst(mark="D01", side="North", width=0.82)
    ...
    def test_label_match(self):
        """Mark label match contributes to score."""
        inst = _inst(mark="D01", side="North", width=0.82)
        elev = _elev(side="North", w=0.82, label="D01")
        enriched, unmatched = correlate_elevation_to_plan([elev], [inst])
        self.assertIsNotNone(enriched[0].height_m)
    ...
    def test_no_instances(self):
        """No instances → all elevations unmatched."""
        elev = _elev()
...
class TestSafetyContract(unittest.TestCase):
    ...
    def test_elevation_source_not_instance(self):
        """elevation_rect is NOT in NON_INSTANCE_SOURCES — it can anchor dedup."""
        self.assertNotIn("elevation_rect", NON_INSTANCE_SOURCES)

    def test_enrichment_never_sets_deduct(self):
        """Enrichment must never set deduct=True."""
        inst = _inst()
        elev = _elev()
        result = _enrich_from_elevation(inst, elev)
    ...
    def test_enrichment_never_creates_instances(self):
        ...
        elevs = [_elev(side="North", w=0.82), _elev(side="North", w=1.2)]
        enriched, _ = correlate_elevation_to_plan(elevs, insts)
        self.assertEqual(len(enriched), 2)

    def test_deduction_status_unchanged(self):
        """Elevation enrichment does not auto-deduct."""
        inst = _inst()
        elev = _elev()
        result = _enrich_from_elevation(inst, elev)
        # Should remain review (or whatever B0's merge computes)
        self.assertIn(result.deduction_status, ("review", "none", "auto_eligible", "derived_eligible"))

    def test_unmatched_elevations_not_in_enriched(self):
        """Unmatched elevation candidates are separated out."""
        inst = _inst(side="North")
        elev_n = _elev(side="North", w=0.82, h=2.1)
    ...
    def test_elevation_geometry_carry(self):
        """Matched instance gets elevation_geometry dict."""
...
class TestLabelExtraction(unittest.TestCase):
    ...
    def test_label_near_rect(self):
        words = [_word("W01", 140, 60, 190, 80)]  # just above rect
        label = _extract_label_near_rect(words, (100, 100, 300, 500))
    ...
    def test_best_distance(self):
        ...
        label = _extract_label_near_rect(words, (100, 100, 300, 500))
        self.assertEqual(label, "D01")  # closer to rect edge
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_correlation_with_no_width_on_instance(self):
        """Instance with no width still matches by side."""
    ...
    def test_correlation_preserves_existing_height(self):
        """If instance already has height from B2, merge still works."""
        inst = _inst(height=2.0, basis=DIMENSION_BASIS_UNKNOWN)
        elev = _elev(h=2.1)
        enriched, _ = correlate_elevation_to_plan([elev], [inst])
        # merge should keep the better dimension bundle
        self.assertIsNotNone(enriched[0].height_m)

    def test_elevation_side_set_on_result(self):
        """elevation_side is set on enriched result even if merge doesn't."""
        inst = _inst(side="")
    ...
    def test_multiple_sides(self):
        ...
        elev_n = _elev(side="North", w=0.82, h=2.1)
        elev_s = _elev(side="South", w=0.82, h=2.4)
        enriched, unmatched = correlate_elevation_to_plan(
            [elev_n, elev_s], [inst_n, inst_s]
        )
        # Both should get different heights
        heights = {e.elevation_side: e.height_m for e in enriched if e.height_m}
        # Can't directly check side on enriched instances, but both should have height
